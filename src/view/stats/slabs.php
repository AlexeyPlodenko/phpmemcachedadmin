<?php use App\Library\Data\Analysis; ?>
...
<div class="size-1" style="float:left;padding-left:9px;">
    <div class="container corner" style="padding:9px;">
            For more information about Memcached slabs stats, <a href="https://github.com/memcached/memcached/blob/master/doc/protocol.txt#L470" target="_blank"><span class="green">see Memcached protocol</span></a>
    </div>
</div>
...
<table class="full-size" cellspacing="0" cellpadding="0">
    <tr>
        <?php $actualSlab = 0 ?>
        <?php foreach($slabs as $id => $slab) : # Slabs parsing ?>
            <?php if (is_numeric($id)) : # If Slab is Used ?>
                <?php if ($actualSlab >= 4) : # Making a new line ?>
                    </tr><tr>
                    <?php $actualSlab = 0 ?>
                <?php endif ?>
                <td<?= $actualSlab > 0 ? ' style="padding-left:9px;"' : '' ?> valign="top">
        ...
        <?php for(; $actualSlab < 4 ; $actualSlab++) : ?>
